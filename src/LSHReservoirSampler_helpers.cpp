...
#pragma omp parallel for private(TB, hashIdx, inputIdx, ct, allocIdx, counter, reservoir_full,     \
    ...
    for (size_t probeIdx = 0; probeIdx < numProbePerTb; probeIdx++) {
        for (size_t tb = 0; tb < _numTables; tb++) {
            ...
            omp_set_lock(_tablePointersLock + tablePointersIdx(_numReservoirsHashed, hashIdx, tb,
            ...
            if (allocIdx == TABLENULL) {
                ...
                _tablePointers[tablePointersIdx(_numReservoirsHashed, hashIdx, tb, _sechash_a,
                                                _sechash_b)] = allocIdx;
            }
            omp_unset_lock(_tablePointersLock + tablePointersIdx(_numReservoirsHashed, hashIdx, tb,
                                                                 _sechash_a, _sechash_b));
